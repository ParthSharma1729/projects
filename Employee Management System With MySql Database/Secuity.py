...
cur = mydb.cursor()
s = "CREATE TABLE IF NOT EXISTS EmpApp(EmpNo integer(4),EmpName varchar(30),EmpSal float(10,2))"
cur.execute(s)
...
def addemp():
    try:
        ...
        if b == "":
            ...
            cur = mydb.cursor()
            s = "INSERT INTO  EmpApp(EmpNo,EmpName,EmpSal) VALUES(%s,%s,%s)"
            b1 = (a, b, c)
            cur.execute(s, b1)
            mydb.commit()
...
def updateemp():
    i = e5.get()
    j = e6.get()
    s = "UPDATE EmpApp SET EmpSal=%s WHERE EmpName=%s"
    u = (j, i)
    cur.execute(s, u)
    x = cur.rowcount
    if x == 0:
...
def search():
    t1.delete("1.0", END)
    s = "SELECT * FROM EmpApp where EmpName LIKE '%s' "
    i = en1.get()
    cur.execute(s % (i))
    result = cur.fetchall()
    st = ""
...
def remove():
    ...
    ent.delete(0, END)
    tet.delete('1.0', END)
    s = "DELETE FROM EmpApp WHERE EmpName=%s"
    cur.execute(s, (rem,))
    x = cur.rowcount
    if x == 0:
        tet.insert(END, "Not Found")
    else:
        tet.insert(END, "Successfully Deleted rows:" + str(x))
...
def dotask():
    ...
    if a == 1:
        ...
        lbl2 = Label(rt, text=" Add   Employee ", relief="ridge", font="Arial 18 bold", fg="white", bg="black")
        lbl2.place(x=110, y=20)

        lbl3 = Label(rt, text="Emp No:", font="Arial 13 bold", fg="white", bg="black")

        lbl3.place(x=50, y=85)

        lbl4 = Label(rt, text="Emp Name:", font="Arial 13 bold", fg="white", bg="black")

        lbl4.place(x=50, y=135)

        lbl5 = Label(rt, text="Salary:", font="Arial 13 bold", fg="white", bg="black")

        lbl5.place(x=50, y=185)
        ...
        btn1 = Button(rt, text="Add Employee", command=addemp, font="Arial 10 bold", bg="red")
        ...
        l2 = Label(rot, text=" Search   Employee ", relief="ridge", font="Arial 18 bold", fg="white", bg="black")
        l2.place(x=90, y=20)

        lbl4 = Label(rot, text="Emp Name:", font="Arial 13 bold", fg="white", bg="black")
        ...
        global rooot
        rooot = Tk()
        rooot.title("Show All Employees")
        w = 400
        h = 390
        ...
        l2 = Label(rooot, text="  Show All Employees  ", relief="ridge", font="Arial 18 bold", fg="white", bg="black")
        l2.place(x=80, y=20)
        ...
        lbl2 = Label(roooot, text=" Update   Employee ", relief="ridge", font="Arial 18 bold", fg="white", bg="black")
        lbl2.place(x=80, y=20)

        lbl3 = Label(roooot, text="Emp ID:", font="Arial 13 bold", fg="white", bg="black")

        lbl3.place(x=50, y=85)

        lbl4 = Label(roooot, text="Emp Name:", font="Arial 13 bold", fg="white", bg="black")

        lbl4.place(x=50, y=135)

        lbl5 = Label(roooot, text="Salary:", font="Arial 13 bold", fg="white", bg="black")
        ...
        l2 = Label(r, text=" Delete Employee ", relief="ridge", font="Arial 18 bold", fg="white", bg="black")
        l2.place(x=100, y=20)

        l3 = Label(r, text="Employee Name", font="Arial 14 bold", fg="white", bg="black")
        l3.place(x=125, y=95)
        global ent
